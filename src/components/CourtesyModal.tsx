...
export function CourtesyModal({
  ...
  // Manejo de visibilidad tanto si se abre por prop o por botón interno
  const isControlled = externalIsOpen !== undefined;
  const showModal = isControlled ? externalIsOpen : internalOpen;
    ...
    <>
      {!isControlled && (
        <button
          ...
          className="px-4 py-2 bg-amber-400 hover:bg-amber-300 text-black font-bold text-xs rounded-xl font-mono transition-all"
        >
          + Emitir Cortesía
        </button>
      )}
                    ...
                    >
                      {ticketTypes && ticketTypes.length > 0 ? (
                        ticketTypes.map((t: any, i: number) => (
                          <option key={i} value={t.name || t.tier_name}>
                            {t.name || t.tier_name}
                          </option>
                        ))
                      ) : (
